/**
 * t3Catopen —— CatOpen JS 源
 * 移动端: 主线程直接执行，不再走 workerpool。
 */
...
import { runHandler } from './worker';

const logger = loggerService.withContext(SITE_TYPE.T3_CATOPEN);
...
class T3CatopenAdapter {
  ...
  private async execCtx<T extends string>(type: T, options?: any): Promise<any> {
    return runHandler(type, options);
  }

  public async init(): ICmsResultPromise['init'] {
    const { request } = await import('@mobile/services/request');
    const { data: code } = await request.request({ url: this.api, method: 'GET' });

    let ext: any = this.ext;
    const { isJsonStr } = await import('@shared/modules/validate');
    const JSON5 = (await import('json5')).default;
    if (isJsonStr(ext)) ext = JSON5.parse(ext);

    await this.execCtx('init', { id: this.id, ext, code });
  }
  ...
  public async home(): ICmsResultPromise['home'] {
    ...
    const classes = rawClassList
      ...
      .filter(
        (item, index, self) =>
          item.type_id && item.type_name && !this.categories?.includes(item.type_name) && self.findIndex((o) => o.type_id === item.type_id) === index,
      );
  ...
  async action(doc: ICmsParams['action']): ICmsResultPromise['action'] {
    ...
    return await this.execCtx('action', { action, value, timeout });
  }

  async proxy(doc: ICmsParams['proxy']): ICmsResultPromise['proxy'] {
    return await this.execCtx('proxy', doc);
  }

  public async runMain(doc: ICmsParams['runMain']): ICmsResultPromise['runMain'] {
    return await this.execCtx('runMain', doc);
  }
}
